...
inline static void add_conn_ht(etcp_serv_t *serv, etcp_serv_conn_t *conn) {
    if (!conn) {
        return;
    }
...
inline static etcp_serv_conn_t *find_conn_ht(etcp_serv_t *serv, int fd) {
    if (fd <= 0) {
        return NULL;
    }
...
// no free conn
inline static void del_conn_ht(etcp_serv_t *serv, int fd) {
    if (fd <= 0) {
        return;
    }
    etcp_serv_conn_t *conn = NULL;
...
// inline static void del_conn_ht(etcp_serv_t *serv) {
//     if (!serv->conn_ht) {
//         return;
//     }
...
// static void free_conn(etcp_serv_t *serv, int fd) {
//     // TODO:
//     return;
// }
...
static etcp_serv_conn_t *init_conn(etcp_serv_t *serv, int cli_fd) {
...
static void accept_cb(struct ev_loop *loop, struct ev_io *watcher, int revents) {
    ...
    etcp_serv_conn_t *conn = init_conn(serv, cli_fd);
    if (!conn) {
        return;
    }
...
etcp_serv_t *etcp_init_server(etcp_serv_conf_t *conf, struct ev_loop *loop, void *user_data) {
    ...
    ev_io_init(serv->accept_watcher, accept_cb, serv->serv_fd, EV_READ);
...
void etcp_server_close_conn(etcp_serv_t *serv, int fd) {
    etcp_serv_conn_t *conn = find_conn_ht(serv, fd);
    if (!conn) {
